...
module Spree
  class Product < Spree::Base
    ...
    after_initialize :ensure_store_products, if: :no_store_products?
    ...
    def default_variant
      @default_variant ||= Rails.cache.fetch(default_variant_cache_key) do
        if Spree::Config[:track_inventory_levels] && variants.in_stock_or_backorderable.any?
          [variants.in_stock, variants.backorderable].flatten.first
        else
    ...
    def ensure_store_products
      return unless new_record?
      self.stores << Spree::Store.default
    end

    def no_store_products?
