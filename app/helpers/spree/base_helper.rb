  ...
  module BaseHelper
    ...
    def og_meta_data
      ...
      if object.is_a? Spree::Product
        ...
        set_availability(og_meta, object)

        price = object.price_in(current_currency)
        if price
          og_meta['og:price']               = "#{price.amount} #{current_currency}"
    ...
    def set_availability(og_meta, product)
      if product.default_variant.in_stock?
        og_meta['og:availability'] = 'in_stock'
      elsif product.default_variant.backorderable?
        og_meta['og:availability'] = 'backorder'
        og_meta['og:availability_date'] = Time.now + 2.weeks.to_i
      else
        og_meta['og:availability'] = 'out_of_stock'
      end
    end
